class WhatsAppReply
  @action: (actions, ticket, article, ui) ->
    ...
    if article.sender.name is 'Customer' && article.type.name is 'whatsapp personal-message'
      actions.push {
        name: 'reply'
        type: 'whatsappPersonalMessageReply'
        icon: 'reply'
        href: '#'
      }
  ...
  @perform: (articleContainer, type, ticket, article, ui) ->
    return true if type isnt 'whatsappPersonalMessageReply'
  ...
  @articleTypes: (articleTypes, ticket, ui) ->
    ...
    return articleTypes if articleTypeCreate isnt 'whatsapp personal-message'
    articleTypes.push {
      name:              'whatsapp personal-message'
      icon:              'whatsapp'
      attributes:        []
      internal:          false,
      features:          ['attachment']
  ...
  @setArticleTypePost: (type, ticket, ui) ->
    return if type isnt 'whatsapp personal-message'
  ...
  @params: (type, params, ui) ->
    if type is 'whatsapp personal-message'
